...
// DFA = deterministic finite automoton is bunch of nodes and an input string
// NFA non-deterministic finite automoton
// Multiplayer, replicate on game instance (since it exists at all times)
class USM_Branch;
class USM_State;
...
enum class EStateMachineCompletionType : uint8
{
	...
	/** Explicit failure - this state is specifically masked as failure/not-accept */
	Rejected,
...
public:
	...
	/** Attempt to run the state with input from the given DataSource object. Will start reading input at DataIndex. Will decrement RemainingSteps and automatically fail after it hits 0. */
...
protected:
	/** Loop. Used when input atom being process isn't recognized. */
	...
	/** Branches to other states. These are in priority order, so the first successful branch will be taken. These run after InstancedBranches. */
	UPROPERTY(EditAnywhere)
